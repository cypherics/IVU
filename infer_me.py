...
def _run(infer_for, model_pth, data_dir, save_dir, stride):
    infer_data = Inference.init_with_parameters(
        ...
        video_parameters={"frame_height": -1, "frame_width": -1, "stride": int(stride)},
        save_dir=save_dir,
        data_dir=data_dir,
    )
    infer_data.run()
...
if __name__ == "__main__":
    _run(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5])
